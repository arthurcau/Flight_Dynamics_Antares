...
| Nominal rail exit 26.37 vs MC 56.9 | Same ascent vehicle/model until any failure event | `simulation.campaign.run_project_campaign`; `simulation.monte_carlo_failure.execute_monte_carlo` | m/s | The orchestrator preferred `monte_carlo_failure.py`; that function changes the returned trajectory to ballistic descent after 200 m AGL and patches only selected fields. | Nominal campaigns select `monte_carlo.py`; failure campaigns are explicit and separately identified. | `test_campaign_prefers_nominal_monte_carlo`. |
